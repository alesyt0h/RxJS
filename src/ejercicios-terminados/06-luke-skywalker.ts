...
import { switchMap, map, tap, mergeAll, mergeMap, concatMap } from 'rxjs/operators';
 ...
 *   Luke Skywalker, llamando el endpoint:   /people/1/    --- Tiene que ser el 2, el 1 no retorna species
 * 
...
(() =>{
    ...
    getRequest(`${SW_API}/people/2/`).pipe(
        ...
        switchMap( resp => { 
            return zip(of(resp), getRequest(resp.species[0])).pipe(
                   map(([personaje,especie]) => ({personaje, especie}))
            )
        })

        // Forma corta
        // switchMap( resp => zip(of(resp), getRequest(resp.species[0])).pipe(
